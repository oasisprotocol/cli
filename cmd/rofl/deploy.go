...
var (
	...
	deployCmd = &cobra.Command{
		...
		Run: func(_ *cobra.Command, _ []string) {
			...
			var doDeployMachine func(*buildRofl.Machine) error
			doDeployMachine = func(machine *buildRofl.Machine) error {
				...
				if err != nil && strings.Contains(err.Error(), "instance not found") {
					if deployReplaceMachine {
						...
						cobra.CheckErr(err)
						return doDeployMachine(machine)
					}

					cobra.CheckErr("Machine instance not found. If your instance expired, run this command with --replace-machine flag to replace it with a new machine.")
				}
				cobra.CheckErr(err)
