...
// parseDotEnv parses a .env formatted string into key/value pairs.
//
// Supports lines like:
//
//	KEY=value
...
func parseDotEnv(data string) (map[string]string, error) {
	...
	for i := 0; i < len(lines); i++ {
		...
		if eq == -1 {
			fmt.Fprintf(os.Stderr, "warning: skipping malformed line %d (no '='): %s\n", i+1, raw)
			continue
		}
		key := strings.TrimSpace(line[:eq])
		if key == "" {
			fmt.Fprintf(os.Stderr, "warning: skipping malformed line %d (empty key)\n", i+1)
			continue
		}
...
func collectQuotedValue(valuePart string, lines []string, startIdx int) (string, int, error) {
	...
	for {
		v, e := parseQuotedValue(combined, delim, unescape)
		if e == nil {
			return v, consumed, nil
		}

		// Only continue if this is the unterminated-quote condition.
		if !strings.HasPrefix(e.Error(), "unterminated quoted value:") {
			return "", 0, fmt.Errorf("line %d: %w", startIdx+1, e)
		}

		// Need another line.
		if startIdx+consumed+1 >= len(lines) {
			return "", 0, fmt.Errorf("line %d: unterminated quoted value", startIdx+1)
		}
		consumed++
