...
func prettyPrintAllowanceDescriptions(
	...
	for _, desc := range allowDescriptions {
		fmt.Fprintf(w, "%s  - %-*s %s", prefix, lenLongest, beneficiaryFieldName, common.PrettyAddressBothFromConsensus(network, desc.beneficiary))
		if desc.self {
			fmt.Fprintf(w, " (self)")
		}
		fmt.Fprintln(w)
...
func prettyPrintAllowances(
	...
	fmt.Fprintf(w, "%s", helpers.FormatConsensusDenomination(network, totalAllowanceAmount))
	fmt.Fprintln(w)
