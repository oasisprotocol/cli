...
type machineShowOutput struct {
	...
	MachineID       roflmarket.InstanceID       `json:"machine_id,omitempty"`
...
var showCmd = &cobra.Command{
	...
	Run: func(_ *cobra.Command, args []string) {
		...
		_, machineName, machineID, _, extraCfg, providerAddr, npa := resolveMachineManifestNpa(args, &roflCommon.ManifestOptions{
			NeedAppID: true,
			NeedAdmin: false,
		})
		out.Name = machineName
		...
		out.Machine, err = conn.Runtime(npa.ParaTime).ROFLMarket.Instance(ctx, client.RoundLatest, *providerAddr, out.MachineID)
		if err != nil {
		...
		out.MachineCommands, err = conn.Runtime(npa.ParaTime).ROFLMarket.InstanceCommands(ctx, client.RoundLatest, *providerAddr, out.MachineID)
		cobra.CheckErr(err)

		out.Provider, err = conn.Runtime(npa.ParaTime).ROFLMarket.Provider(ctx, client.RoundLatest, *providerAddr)
		cobra.CheckErr(err)
		...
		case true:
			...
			out.Replica, _ = conn.Runtime(npa.ParaTime).ROFL.AppInstance(ctx, client.RoundLatest, out.Provider.SchedulerApp, pk)
		default:
		}
		...
		if common.OutputFormat() == common.FormatJSON {
			...
			prettyPrintMachine(npa, &out, extraCfg)
		}
	},
}

// prettyPrintMachine prints a compact human-readable info of the ROFL machine.
func prettyPrintMachine(npa *common.NPASelection, out *machineShowOutput, extraCfg *roflCmdBuild.AppExtraConfig) {
	...
	if out.Replica != nil {
		if proxyDomain, ok := out.Replica.Metadata[scheduler.MetadataKeyProxyDomain]; ok {
			numericMachineID := binary.BigEndian.Uint64(out.MachineID[:])
			proxyDomain = fmt.Sprintf("m%d.%s", numericMachineID, proxyDomain)

			fmt.Printf("Proxy:\n")
			fmt.Printf("  Domain: %s\n", proxyDomain)

			prettyPrintMachinePorts(extraCfg, out.Machine.Deployment.AppID, out.Machine, proxyDomain)
		}
	}
