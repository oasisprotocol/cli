...
func maybeDownloadArtifact(kind, uri, knownHash string) string {
	...
	if knownHash != "" {
		...
		if artifactHash != knownHash {
			cobra.CheckErr(fmt.Errorf("hash mismatch for %s artifact (expected: %s got: %s)", knownHash, artifactHash))
		}
	}
...
FILES:
	for {
		...
		case tar.TypeReg:
			...
			if _, err = io.Copy(fh, rd); err != nil {
				fh.Close()
...
func copyFile(src, dst string, mode os.FileMode) error {
	sf, err := os.Open(src)
	if err != nil {
		return fmt.Errorf("failed to open '%s': %w", err)
	}
	defer sf.Close()
...
func createExt4Fs(fn, dir string) (int64, error) {
	...
	fsSize = fsSize / 1024        // Convert to kilobytes.
	fsSize = (fsSize * 140) / 100 // Scale by overhead factor of 1.4.
