...
func TestPrettyResolvedAddressWith(t *testing.T) {
	require := require.New(t)
	...
	t.Run("eth preferred when provided", func(t *testing.T) {
		ctx := AddressFormatContext{
			Names: types.AccountNames{
				nativeAddr.String(): "my",
			},
		...
		require.Equal("my ("+ethAddr.Hex()+")", PrettyResolvedAddressWith(ctx, nativeAddr, ethAddr))
	})

	t.Run("eth preferred when mapped", func(t *testing.T) {
		ctx := AddressFormatContext{
			Names: types.AccountNames{
				nativeAddr.String(): "my",
			},
		...
		require.Equal("my ("+ethAddr.Hex()+")", PrettyResolvedAddressWith(ctx, nativeAddr, nil))
	})
	...
	t.Run("native fallback when eth unknown", func(t *testing.T) {
		...
		require.Equal("my ("+nativeAddr.String()+")", PrettyResolvedAddressWith(ctx, nativeAddr, nil))
	})

	t.Run("unknown returns preferred input", func(t *testing.T) {
		ctx := AddressFormatContext{
			Names: types.AccountNames{},
			Eth:   map[string]string{},
		}

		require.Equal(nativeAddr.String(), PrettyResolvedAddressWith(ctx, nativeAddr, nil))
		require.Equal(ethAddr.Hex(), PrettyResolvedAddressWith(ctx, nativeAddr, ethAddr))
	})
}
